...
describe("LFGVesting", function () {
  ...
  before("Deploy contract", async function () {
    try {
      ...
      LFGToken = await LFGTokenArt.new("LFGToken", "LFG", 10000, owner);
      LFGVesting = await LFGVestingArt.new(owner);
      await LFGVesting.setLFGToken(LFGToken.address, {from: owner});
  ...
  it("test setVestingPool method", async function () {
    ...
    await LFGVesting.setVestingInfo(1, 5, today, 60, {from:owner});
  ...
  it("test addWhiteLists method", async function () {
    await LFGVesting.addWhitelists([accounts[1]], [vestingAmount], [1], [unlockedAmount], {from: owner});
